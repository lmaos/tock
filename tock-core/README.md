...
Tock 是一个面向 JVM 的分布式定时器内核：**一个 Master 负责调度，节点按需作为 Worker 执行任务**。  
当前仓库提供两种运行模式：
...
- Windows 本机 memory-only 严格 A/B 下，`high-precision` 的 `worker-chain avg abs skew` 为 **0.28ms**，`default-worker` 为 **4.31ms**
- Windows 本机 Redis 高精度 Worker 为 **0.44ms**，`default-worker` 为 **8.16ms**
- Windows 本机内存模式吞吐约 **5708.78 tasks/s**
- Windows 本机 Redis 模式吞吐约 **4200.35 tasks/s**
