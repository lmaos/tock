...
1. 启动 `workerExecutor`
2. 给 `TockMaster` 注册监听器
3. 给当前节点注册 `NodeListener`
4. 启动 `jobRegistry` / `jobStore` / `workerQueue`
5. 启动 `register`
6. 当前节点心跳先建立起来，Worker 再开始消费
7. 选主成功后，Scheduler 启动；失去主身份则停止调度
...
- 业务线程里拿到的时间会优先来自 `TimeSnapshot`，系统时间源也会返回统一的快照对象
...
- **本次等待 delay 的计算基准**：继续使用当前时间

这样避免了 Redis 模式下的重复命中当前时间窗、1ms 级连锁重排和同秒连发。

另外，Worker 现在会等心跳真正建立后再恢复消费，这样刚 resume 时不会马上被心跳失败又打回去。
...
- 心跳链路现在区分首次建立和后续恢复，Worker 的启动感知也更稳定了
- Worker 的时间快照会贯穿同一个任务的排队、等待、执行业务和收尾过程，业务侧看到的是同一份时间种子

详细数据见 [../PERFORMANCE.md](../PERFORMANCE.md)。
