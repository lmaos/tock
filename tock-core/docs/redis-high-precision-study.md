...
> **状态说明**
>
> 这份文档记录的是一次**历史性暴露问题**的专项研究，不代表当前版本仍存在同样缺陷。  
> 当前代码已经完成修复：**高精度时间轮在分布式时间源下，如果业务没有手动覆盖 `advanceNanos`，默认会自动切到 `1ms`**。  
> 当前版本的最终能力状态请优先参考 [`../PERFORMANCE.md`](../PERFORMANCE.md)。

本文讨论的是：**为什么 `redis-high-precision` 在先前的性能报告里一度看起来比 `redis-default-worker` 更差，以及这个历史现象当时是如何被定位和修复的。**
...
1. **之前那组 `5.69ms vs 7.94ms` 的差距不是稳定结论。**  
   在更聚焦的 10 轮分段计时 study 里，问题被收敛到了“默认 advance 参数不适配”，不是 Redis 模式天然更差。
...
这也是为什么当前代码把“分布式时间源 + 未手动覆盖”的默认值改成了 `1ms`。
...
- 如果 `timeProvider` 是 `SystemTimeProvider`，高精度时间轮保持 `200us` 本地默认值
- 如果 `timeProvider` 来自 Redis 或其他非系统时间源，并且业务 **没有** 显式调用 `setAdvanceNanos(...)`
  - Tock 会自动把高精度时间轮默认 advance 调整到 `1ms`
...
这次 study **没有 Linux 对照结果**，因为当前执行环境只有 Windows。
