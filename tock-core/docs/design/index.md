...
这组文档按 `src/main/java/com/clmcat/tock` 的目录结构组织，目标是把每个模块的职责、核心类、调用关系和扩展点讲清楚。
...
5. **调度提前，Worker 本地执行**：Master 提前把任务送进队列，Worker 用本机定时器等到点再执行业务，减少网络抖动对精度的影响
6. **统一时间基准**：默认使用服务器当前时间；如果配置时间同步，则由 `time` 模块把调度和 Worker 的时间基准拉齐
7. **健康只管健康**：健康链路只负责发现、心跳和过期通知，不直接掺入调度或业务执行
